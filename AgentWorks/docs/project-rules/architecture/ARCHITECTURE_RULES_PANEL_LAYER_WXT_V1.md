# ARCHITECTURE RULES (Panel Layer Runtime, WXT Browser Extension Profile) V1

## 1) 문서 역할
- 이 문서는 WXT + TypeScript 브라우저 확장 프로젝트에서 `LayeredPanel` runtime 규칙을 정의하는 문서다.
- 이 문서는 `ARCHITECTURE_RULES_PANEL_SECTION_WXT_V1.md`의 `LayeredPanel` 개념을 실제 runtime 계약으로 확장한다.
...
- 이 문서는 `popup`, `options`, `sidepanel`, content script가 만든 user-facing DOM root 같은 WXT UI surface에 공통 적용한다.

## 3) 핵심 원칙
- `BasePanel`과 `LayeredPanel`은 공용 `Panel` 개념을 WXT에서 역할별로 나눈 플랫폼 프로파일 용어다.
- surface 내부 화면 전환은 `BasePanel`만 관장한다.
- `LayeredPanel`은 surface entrypoint를 직접 소유하지 않는다.
...
- z-order는 layered stack 내부 순서를 뜻하며, 서로 다른 WXT surface를 가로지르는 전역 z-order 체계로 두지 않는다.
...
- app shell, entrypoint bootstrap, background runtime은 layered stack 상태를 직접 소유하지 않는다.
...
- content script surface에서는 host가 extension mount root 내부에서만 layered panel을 렌더링하는 것을 기본으로 한다.
- host page DOM의 임의 위치에 자유 노드를 흩뿌리는 구조는 프로젝트 문서에서 별도 승인된 경우가 아니면 기본 패턴으로 두지 않는다.
...
- WXT `popup` surface는 브라우저 blur나 popup close로 전체 surface가 사라질 수 있으므로, 브라우저가 popup을 닫아주는 동작을 exit animation completion 대체 수단으로 취급하지 않는다.
...
- WXT UI surface도 DOM 기반이므로 focus trap, restore, keyboard priority는 웹 기준 DOM 포커스 정책으로 다룬다.
...
- WXT 기준 anchor metadata는 `anchorRect` 같은 viewport 좌표 정보로 해석하는 편을 권장한다.
- popup/options/sidepanel surface에서는 현재 extension viewport 기준으로 위치를 계산한다.
- content script surface에서는 host page scroll, zoom, layout reflow에 따라 anchor 좌표를 재측정할 수 있어야 한다.
...
- overlay 내부 전환은 surface 전환이 아니다.
...
- 같은 surface 안에서 overlay를 많이 사용하더라도 app-level `BasePanel` 교체 규칙 자체는 바뀌지 않는다.
- `popup`, `options`, `sidepanel` 같은 entry surface 자체가 바뀌는 것은 layered 전환이 아니며, extension runtime의 다른 진입으로 본다.
...
- `LayeredPanel`이 surface entrypoint를 직접 대표하려는 구조
- `LayeredPanel`이 특정 `BasePanel`을 직접 import해 강결합되는 구조
- `BasePanel` 전환 시 기존 layered stack가 계속 살아남는 구조
- content script surface에서 extension root 바깥 host page DOM에 overlay를 무질서하게 흩뿌리는 구조
